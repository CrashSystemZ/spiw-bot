...
import { MetadataResolveOptions, MetadataPlatform, NormalizedMetadataUrl, ResolvedMetadata } from "../../types/metadata.js";
...
function extractInstagramState(html: string): unknown {
  const sharedData = metaFirst(html, "og:description") ? undefined : undefined;
...
function pickBestUrl(node: InstagramMediaNode): string | undefined {
  if (node.is_video) {
    return pickFirstHttpUrl(node.video_url);
  }
  return firstDefined(
    pickFirstHttpUrl(node.display_url),
    node.display_resources?.slice().reverse().map((entry) => pickFirstHttpUrl(entry.src)).find(Boolean),
  );
}
